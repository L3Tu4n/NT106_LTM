...
func ResetPasswordAndSendEmail(db *sqlx.DB, email, newPassword string) error {
	...
	// Thông tin xác thực Gmail
	emailFrom := "SEND_EMAIL"         // Địa chỉ email của bạn
	password := "SEND_EMAIL_PASSWORD" // Mật khẩu email của bạn
...
func GetProfileByEmail(email string, db *sqlx.DB) (*User, error) {
	...
	var profile User
	err := db.Get(&profile, "SELECT ID, NAME, EMAIL, PASSWORD, PHONE FROM Users WHERE EMAIL = ?", email)
	if err != nil {
		return nil, err
	}
